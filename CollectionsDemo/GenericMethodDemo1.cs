...
{
    class DataStore1<T> //generic class
    {
        private T[] _data = new T[10]; //generic array

        public void AddOrUpdate(int index, T item) //generic method which accepts generic type for item 
        {
            if (index >= 0 && index < 10)
                _data[index] = item;
        }

        public T GetData(int index) //generic method with generic return type
        {
            if (index >= 0 && index < 10)
                return _data[index];
            else
